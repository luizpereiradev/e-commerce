...
describe("testes /user", () => {
  describe("GET /user", () => {
    describe("em caso de sucesso", () => {
      ...
      it("deve retornar um array de usuários e enviar status 200", async () => {
        ...
        expect((Logger.save as sinon.SinonStub).calledWith("getAll() success"))
          .to.be.true;
      });
    });
    ...
    describe("em caso de erro no banco de dados", () => {
      ...
      it("deve retornar a mensagem do erro e enviar status 500", async () => {
        ...
        expect((Logger.save as sinon.SinonStub).calledWith("getAll() fail")).to
          .be.true;
      });
    });
  });
  ...
  describe("PUT /user/:id", () => {
    describe("caso o usuário exista", () => {
      ...
      it("deve retornar o usuário atualziado e enviar status 200", async () => {
        ...
        expect((Logger.save as sinon.SinonStub).calledWith("update() success"))
          .to.be.true;
      });
    });
    ...
    describe("caso o usuário não exista", () => {
      ...
      it("deve retornar a mensagem do erro e enviar status 404", async () => {
        ...
        expect((Logger.save as sinon.SinonStub).calledWith("update() fail")).to
          .be.true;
      });
    });
  });
  ...
  describe("POST /user", () => {
    describe("em caso de sucesso", () => {
      ...
      it("deve criar um novo usuário e enviar status 201", async () => {
        ...
        expect((Logger.save as sinon.SinonStub).calledWith("create() success"))
          .to.be.true;
      });
    });
    ...
    describe("em caso de erro no banco de dados", () => {
      ...
      it("deve retornar a mensagem do erro e enviar status 500", async () => {
        ...
        expect(body).to.have.property("error");
        expect(body.error.message).to.be.equal("db error");
        expect((Logger.save as sinon.SinonStub).calledWith("create() fail")).to
          .be.true;
      });
    });
  });
  ...
  describe("DELETE /user/:id", () => {
    describe("caso o usuário exista", () => {
      before(() => {
        sinon
          .stub(UserRepository.prototype, "delete")
          .resolves(fakeData.del.mock);
        sinon
          .stub(UserRepository.prototype, "getById")
          .resolves(fakeData.del.getByIdMock);
        sinon.stub(Logger, "save").resolves();
      });
      ...
      it("deve retornar uma mensagem de sucesso e enviar status 200", async () => {
        ...
        expect(body).to.be.an("object");
        expect((Logger.save as sinon.SinonStub).calledWith("delete() success"))
          .to.be.true;
      });
    });
    ...
    describe("caso o usuário não exista", () => {
      ...
      it("deve retornar a mensagem de erro e enviar status 404", async () => {
        ...
        expect(body).to.have.property("error");
        expect(body.error.message).to.be.equal("user not found");
        expect((Logger.save as sinon.SinonStub).calledWith("delete() fail")).to
          .be.true;
      });
    });
  });
